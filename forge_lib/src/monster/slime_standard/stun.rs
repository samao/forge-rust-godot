...
impl IStateTrait for SlimeStunState {
...
impl SlimeStunState {
    ...
    fn exit(&mut self) {
        if let Some(mut board) = self.get_blackboard() {
            board.bind_mut().set_can_decide(true);
        }
    }
    #[func]
    fn update(&mut self, delta: f32) {
        ...
        if let Some(mut enemy) = self.get_owner_node() {
            ...
            velocity.x = self.vel_x * (1.0 - self.timer / self.duration) as f32;
            enemy.set_velocity(velocity);

            if self.timer >= self.duration
                && let Some(mut board) = self.get_blackboard()
            {
                board.bind_mut().set_can_decide(true);
            }
        }
